...
function getDataFromAPI(searchTerm) {
  ...
  $.getJSON(GIPHY_URL, query, function(data) {
    ...
    if (data) {
      ...
      resultElement += '<img class="responsive-image" src = "' + results.image_original_url + '">';
...
function renderTask(task) {
  var taskElement =
    ...
    '<button type="button" class = "done-button" name="button" title = "mark completed"><i class="fa fa-check fa-3x" aria-hidden="true"></i></button>' +
    '<button type ="button" class ="reset-button" id="reset" name="reset" title = "delete item"><i class="fa fa-trash-o fa-3x" aria-hidden="true"></i></button>' +
    '<button type = "button" class = "start-timer" title = "reset time">' +
